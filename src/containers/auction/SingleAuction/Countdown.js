...
export default function CountdownComponent({ countdownDetails }) {
  ...
  useEffect(() => {
    function setCountTime() {
      const countDownTime = moment(countdownDetails.endDate).valueOf() - new Date().getTime();
  ...
  return (
    <div className={styles.period}>
      {moment(countdownDetails.startDate).format('D MMM Y')}
      <div className={styles['arrow-icon']}><ArrowRight /></div>
      {moment(countdownDetails.endDate).format('D MMM Y')}
      <span
        className={classNames('icon-arrow-repeat', styles['cricle-icon'])}
        onClick={handleShowCountdown}
      />
